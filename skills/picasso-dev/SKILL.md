...
5. 在需要时补充调用 `picasso-dev-design-system`、`picasso-dev-ui-review`
6. 在需要时补充调用 `picasso-dev-methods`，收敛方法论执行方式
...
- 生成或改写 GitHub 开源 README，并希望按统一模板和门禁输出
...
13. `../../shared/references/readme/README.md`
14. 根据任务类型读取对应工作流：
   ...
   - README 生成：`../../shared/workflow/open-source-readme.md`
...
若当前运行端不可识别 `ui-ux-pro-max`，UI 路由只允许回退到仓库内置 `shared/references/design/`，不得改用 `frontend-design` 或其他通用设计 skill 兜底。
...
- 改写现有 README 为标准开源版本
- 给其他模型提供 README 生成模板
- 校验 README 是否满足开源首页要求

对应入口：

- `../../shared/workflow/open-source-readme.md`

### 调用 `picasso-dev-design-system`

当任务涉及以下任一场景时调用：

- 页面需要明确风格方向
- 需要给出色板、字体、token、组件层级
- 需要补动效、反馈、可访问性要求

### 调用 `picasso-dev-ui-review`

当任务涉及以下任一场景时调用：

- 已有 UI 文档，需要做页面还原评审
- 已有本地页面、截图或录屏，需要做交互回归
- 需要输出 UI 差异清单和回流建议
